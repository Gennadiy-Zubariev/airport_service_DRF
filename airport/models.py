...
class Crew(models.Model):
    ...
    role = models.CharField(
        max_length=63, choices=Role.choices, default=Role.FLIGHT_ATTENDANT
    )
...
class Route(models.Model):
    ...
    def __str__(self):
        return (
            f"FROM {self.source} - TO {self.destination} ({self.distance} km.)"
        )
...
class Order(models.Model):
    ...
    def __str__(self):
        return (
            f"Order №{self.id} - by {self.user}(created at {self.created_at})"
        )
...
class Ticket(models.Model):
    ...
    def __str__(self):
        return (
            f"Ticket {self.flight.route} (row: {self.row}, seat: {self.seat})"
        )
